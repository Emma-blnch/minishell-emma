...
/*   Updated: 2025/02/26 08:40:23 by eblancha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	parent_process(t_shell *data, t_cmd *cmd, int *pip)
{
	close(pip[1]);
	if (cmd->infile >= 0)
		close(cmd->infile);
...
static bool	exec_cmd(t_shell *data, t_cmd *cmd, int *pip)
{
	g_signal_pid = fork();
	if (g_signal_pid < 0)
...
static void	wait_all(t_shell *data)
{
	int		status;
	int		pid;
	int		len;
