...
/*   Updated: 2025/02/11 10:49:48 by ahamini          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_token(t_shell *shell, int lexem, char letter)
{
	...
	if (!token)
		return (set_error(MALLOC_FAIL, shell, "Failed to allocate token")); /*il faut free tout ce qui a ete allouer et fermer le programme*/
	token->content = ft_calloc(sizeof(char), 2);
	if (!token->content)
		return (set_error(MALLOC_FAIL, shell, "Failed to alloc token data")); /*same*/
	token->lexem = lexem;
	token->letter = letter;
	...
	if (!node)
		return (set_error(MALLOC_FAIL, shell, "Failed to alloc token node")); /*same*/
