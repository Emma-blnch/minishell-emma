...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/11 10:49:34 by ahamini           #+#    #+#             */
/*   Updated: 2025/02/11 10:49:38 by ahamini          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void syntax_error(t_token *token)
{
    ...
    else if (token->letter == 'R')  // Si c'est notre représentation de '>>'
...
void is_missing_redirection(t_shell *shell, t_list *node)
{
    ...
    if (redirection->letter == 'R' && next_token->lexem != WORD)  // Vérifier 'R' pour '>>'
    {
        syntax_error(next_token);
        shell->critical_er = SYNTAX_ERROR;
        return ;
    }
    else if (next_token->lexem != WORD)
    {
        syntax_error(next_token);
        shell->critical_er = SYNTAX_ERROR;
        return ;
